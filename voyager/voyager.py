...
class Voyager:
    def __init__(
        ...
        action_agent_model_name: str = "qwen-plus",
        action_agent_temperature: float = 0,
        action_agent_task_max_retries: int = 4,
        action_agent_show_chat_log: bool = True,
        action_agent_show_execution_error: bool = True,
        curriculum_agent_model_name: str = "qwen-plus",
        curriculum_agent_temperature: float = 0,
        curriculum_agent_qa_model_name: str = "qwen-plus",
        curriculum_agent_qa_temperature: float = 0,
        curriculum_agent_warm_up: Dict[str, int] = None,
        curriculum_agent_core_inventory_items: str = r".*_log|.*_planks|stick|crafting_table|furnace"
        r"|cobblestone|dirt|coal|.*_pickaxe|.*_sword|.*_axe",
        curriculum_agent_mode: str = "auto",
        critic_agent_model_name: str = "qwen-plus",
        critic_agent_temperature: float = 0,
        critic_agent_mode: str = "auto",
        skill_manager_model_name: str = "qwen-plus",
        ...
        # init env
        logger.info(f"Setting up Minecraft environment (port={mc_port}, server_port={server_port})")
        self.env = VoyagerEnv(
            mc_port=mc_port,
        ...
        # set openai api key
        logger.debug("Setting API keys from environment")
        os.environ["OPENAI_API_KEY"] = openai_api_key
        os.environ["DASHSCOPE_API_KEY"] = dashscope_api_key
        os.environ["DASHSCOPE_API_BASE"] = dashscope_base_url

        # init agents
        logger.info(f"Initializing Action Agent (model={action_agent_model_name}, temp={action_agent_temperature})")
        ...
        if not self.action_agent.test_yourself():
            raise RuntimeError("Action Agent failed the self-test")
        self.action_agent_task_max_retries = action_agent_task_max_retries
        
        logger.info(f"Initializing Curriculum Agent (model={curriculum_agent_model_name}, mode={curriculum_agent_mode})")
        ...
        if not self.curriculum_agent.test_yourself():
            raise RuntimeError("Curriculum Agent failed the self-test")
        
        logger.info(f"Initializing Critic Agent (model={critic_agent_model_name}, mode={critic_agent_mode})")
        ...
        if not self.critic_agent.test_yourself():
            raise RuntimeError("Critic Agent failed the self-test")
        
        logger.info(f"Initializing Skill Manager (model={skill_manager_model_name}, top_k={skill_manager_retrieval_top_k})")
        ...
        if not self.skill_manager.test_yourself():
            raise RuntimeError("Skill Manager failed the self-test")
        
        logger.info(f"Initializing EventRecorder (ckpt_dir={ckpt_dir}, resume={resume})")
        ...
        logger.info("Voyager initialization complete")
        logger.info("=" * 50)

    def reset(self, task, context="", reset_env=True):
        logger.info(f"Resetting Voyager for task: '{task}'")
        ...
        if reset_env:
            logger.debug("Resetting environment (soft mode)")
            self.env.reset(
                options={
                    "mode": "soft",
        ...
        logger.debug(f"Setting difficulty: {difficulty}")
        ...
        logger.info(f"Retrieved {len(skills)} relevant skills for context")
    ...
    def close(self):
        logger.info("Closing Voyager and cleaning up resources")
        self.env.close()
    ...
    def step(self):
        ...
        logger.debug(f"Step {self.action_agent_rollout_num_iter + 1} for task: '{self.task}'")
        ai_message = self.action_agent.llm(self.messages)
        logger.info(f"\033[34m****Action Agent ai message****\n{ai_message.content}\033[0m")
        logger.debug(f"AI response length: {len(str(ai_message.content))} chars")
        ...
        if isinstance(parsed_result, dict):
            logger.info("Executing generated code in Minecraft environment")
            ...
            logger.info("Running critic agent to evaluate task success")
            ...
            logger.info(f"Task evaluation: {'SUCCESS' if success else 'FAILED'}")
            if critique:
                logger.debug(f"Critique: {critique[:200]}{'...' if len(critique) > 200 else ''}")
    ...
    def learn(self, reset_env=True):
        logger.info("=" * 50)
        logger.info("Starting Voyager learning loop")
        logger.info("=" * 50)
        
        if self.resume:
            # keep the inventory
            logger.info("Resuming from checkpoint, keeping inventory")
            self.env.reset(
                options={
                    "mode": "soft",
                    "wait_ticks": self.env_wait_ticks,
            ...
            # clear the inventory
            logger.info("Starting fresh, clearing inventory")
            self.env.reset(
                options={
                    "mode": "hard",
                    "wait_ticks": self.env_wait_ticks,
        ...
        while True:
            if self.recorder.iteration > self.max_iterations:
                logger.warning(f"Iteration limit reached ({self.max_iterations}), stopping learning loop")
                print("Iteration limit reached")
                break
            
            logger.info(f"--- Iteration {self.recorder.iteration}/{self.max_iterations} ---")
            task, context = self.curriculum_agent.propose_next_task(
                events=self.last_events,
                chest_observation=self.action_agent.render_chest_observation(),
                max_retries=5,
            )
            logger.info(f"Proposed task: '{task}'")
            logger.debug(f"Task context: {context[:150]}{'...' if len(context) > 150 else ''}")
            print(
            ...
            try:
                ...
                logger.error(f"Rollout failed with exception: {type(e).__name__}: {str(e)}", exc_info=True)
                ...
                logger.info("Resetting environment after error")
            ...
            if info["success"]:
                logger.info(f"Task '{task}' completed successfully, adding to skill library")
                self.skill_manager.add_new_skill(info)
            else:
                logger.warning(f"Task '{task}' failed")

            self.curriculum_agent.update_exploration_progress(info)
            
            completed_count = len(self.curriculum_agent.completed_tasks)
            failed_count = len(self.curriculum_agent.failed_tasks)
            logger.info(f"Progress: {completed_count} completed, {failed_count} failed")
            print(
